    ...
    uint32_t LBA, uint32_t offset, uint32_t old_PBA, uint32_t old_PPA)
{
	...
	uint32_t old_PPA_zero = old_PBA * ppb_;
	...
	for (int i=0; i<ppb_; ++i) {
		if (BM_IsValidPage(BM, old_PPA_zero+i)) {
			if (i != offset) {
				...
				temp_set[i] = SRMA_load(sram_table, i, old_PPA_zero);
			}
		}
	}
	// Here, start
...
void block_GC()
{
	...
	while (victimSeg_PBA != victimBlock->PBA)
		printf("NONONO!");
	...
	sram_value = (PTR*)malloc(sizeof(PTR) * pps_);
	sram_table = (block_sram*)malloc(sizeof(block_sram) * ppb_);
	for (i=0; i<pps_; ++i)
		sram_value[i] = NULL;
	for (int i=0; i<ppb_; ++i) {
