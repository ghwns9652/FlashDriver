...
/*
static void* lower_bulk_io(int type, uint32_t ppa, uint32_t size, value_set *value, bool async, algo_req* const req){
    static uint32_t first_ppa = UINT32_MAX;
    static uint32_t prev_ppa = UINT32_MAX;
    ...
    if(req){
        isflush = req->parents->isflush;
#ifdef KPRINT
        printf("lower_bulk_io : %d\n", req->parents->key);
#endif
    }

    if(size == 0){
        if(first_ppa != UINT32_MAX){
            goto do_lower_io;
        }
        else{
            fprintf(stderr, "lower_bulk_io input size error\n");
            return NULL;
        }
    }
...
do_lower_io:
    if(first_ppa == UINT32_MAX)
        first_ppa = ppa;
    bsize = sub_total*PAGESIZE;
    a_reqs[0]->sub_algo_req = a_reqs;
    a_reqs[0]->ori_end_req = a_reqs[0]->end_req;
    a_reqs[0]->end_req = lower_bulk_end_req;
    a_reqs[0]->size = bsize;
    if(type==0)
        lower_read(first_ppa, bsize, a_reqs[0]->parents->value, async, a_reqs[0]);
    else if(type==1)
        lower_write(first_ppa, bsize, a_reqs[0]->parents->value, async, a_reqs[0]);

    first_ppa = UINT32_MAX;
    prev_ppa = UINT32_MAX;
    sub_total = 0;
    a_reqs = NULL;

    if(!isflush){
        lower_bulk_io(type, ppa, size, value, async, req);
    }
