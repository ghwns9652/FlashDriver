...
#ifdef AIOTHPOOL
void aio_wrap(void *arg, int id) {
    struct aio_req *req = (struct aio_req*)arg;
    switch(req->type){
        case AIOPUSH:
            aio_push_data(req->ppa, req->size, req->value, req->async, req->upper_req);
            break;
        case AIOPULL:
            aio_pull_data(req->ppa, req->size, req->value, req->async, req->upper_req);
            break;
        case AIOTRIM:
            aio_trim_block(req->ppa, req->async);
            break;
        default:
            printf("aio_req type error\n");
            break;
    }
}
#endif
...
void *aio_make_push(uint32_t PPA, uint32_t size, value_set* value, bool async, algo_req *const req){
    struct aio_req *io_req = (struct aio_req*)malloc(sizeof(struct aio_req));
    io_req->type = AIOPUSH;
    io_req->ppa = PPA;
    io_req->size = size;
    io_req->value = value;
    io_req->async = async;
    io_req->upper_req = req;

	while(thpool_num_threads_working(thpool)>=NUM_THREAD);
	thpool_add_work(thpool, aio_wrap, (void *)io_req);
